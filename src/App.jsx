  ...
  return <section className="phase32-page">
    <div className="phase32-hero panel">
      <div>
        ...
        <p className="phase32-rules">Cada partido vale máximo <b>3 puntos</b>. El primer criterio es acertar el <b>clasificado a la siguiente fase</b>, aunque el usuario haya previsto penales y el equipo clasifique de forma directa.</p>
        <div className="phase32-points-rules">
          <article><strong>+2</strong><span>Clasificado directo</span><small>Si el partido no llega a penales y acierta el equipo que pasa de ronda.</small></article>
          <article><strong>+1</strong><span>Marcador exacto</span><small>Si acierta los goles exactos del partido.</small></article>
          <article><strong>+1</strong><span>Bonus penales</span><small>Si el partido llega a penales y acierta el ganador por penales.</small></article>
          <article><strong>3 máx.</strong><span>Con penales</span><small>Si hay penales: +1 clasificado, +1 marcador exacto y +1 penales.</small></article>
    ...
    <div className="phase32-matches">
      {orderedMatches.map((match,index)=>{
        ...
        return <article className="phase32-match-card" key={match.id}>
          ...
          <div className="phase32-hits"><span>Clasificado <b className={ev.winnerHit===null?'pending-hit':ev.winnerHit?'hit-ok':'hit-bad'}>{hitIcon(ev.winnerHit)}</b></span><span>Resultado <b className={ev.scoreHit===null?'pending-hit':ev.scoreHit?'hit-ok':'hit-bad'}>{hitIcon(ev.scoreHit)}</b></span><span>Penales <b className={ev.penaltyHit===null?'pending-hit':ev.penaltyHit?'hit-ok':'hit-bad'}>{hitIcon(ev.penaltyHit)}</b></span></div>
        </article>
      })}
    </div>

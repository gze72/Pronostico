...
import { adminPhaseControl, deleteParticipantAndForecast, getAppSettings, getForecast, getMatches, getRealScores, listParticipantsWithForecasts, loginOrCreateParticipant, saveForecast, saveParticipantScore, saveRealScore, supabase, syncResultsAndScores, getDailyEditorialSummary, getPhase32Forecast, savePhase32Forecast, getPhase32Results,
  getPhase32Penalties, savePhase32Result } from './lib/storage';
import './styles.css';
...
function App(){
  ...
  const [view,setView] = useState('pronostico16');
  ...
  const [phase32Penalties,setPhase32Penalties] = useState({});
  useEffect(()=>{ 
    async function boot(){
  ...
  useEffect(()=>{ if(participant) getPhase32Forecast(participant.id).then(f=>{ setPhase32Predictions(f?.predictions || {}); setPhase32Status(f?.status || (f?.confirmed ? 'confirmed' : 'empty')); }); },[participant]);
  useEffect(()=>{
  ...
  const phase32Score = useMemo(() => calculatePhase32Score(phase32Matches,phase32Predictions,phase32RealScores,currentPhase32Penalties), [phase32Matches,phase32Predictions,phase32RealScores]);
  const publicRankingRows = useMemo(() => buildPremiumRankingRows(rankingRows,matches,realScores), [rankingRows,matches,realScores]);
  const phase32RankingRows = useMemo(() => buildPremiumPhase32RankingRows(rankingRows,phase32Matches,phase32RealScores,phase32Penalties), [rankingRows,phase32Matches,phase32RealScores,phase32Penalties]);
  ...
  const persistPhase32 = async (confirmed=false) => {
    try {
      if (phase32Locked(appSettings, phase32Status, phase32Matches)) throw new Error('El Pronóstico 16° está bloqueado. Solo ADMIN puede habilitarlo nuevamente.');
  ...
  return <div className={`app-shell ${sidebar ? 'sidebar-open' : 'sidebar-closed'}`}>
    ...
    <aside className={`sidebar ${sidebar?'open':''}`}><button className="sidebar-close" onClick={()=>setSidebar(false)} aria-label="Cerrar menú"><X size={18}/><span>Cerrar</span></button>
      ...
      <nav>
        ...
        <button className={view==='pronostico16'?'active':''} onClick={()=>{setView('pronostico16'); setSidebar(false)}}><Trophy/> Pronóstico 16°</button>
...
function maxPossiblePhase32Points(score) {
  return (score?.evaluatedMatches || 0) * 3;
}

function scorePercentPhase(score) {
  const possible = maxPossiblePhase32Points(score);
  const earned = score?.totalPoints || 0;
  return possible > 0 ? Math.round((earned / possible) * 100) : 0;
}
...
function buildPremiumPhase32RankingRows(rows,phase32Matches,phase32RealScores){
  return rows.map(r=>{
...
function PremiumRankingReport({rows, phaseLabel='FASE 2 · Pronóstico 16°', showPenalty=true}){
  const leader=rows[0];
  const maxPoints=rows.reduce((max,r)=>Math.max(max,r.possible||0),0);
  const evaluated=showPenalty ? (maxPoints>0?Math.round(maxPoints/3):0) : (maxPoints>0?Math.round(maxPoints/2):0);
  ...
  const penaltyTotal=leader?.score?.penaltyPoints || 0;
  return <section className="premium-ranking-report"><div className="premium-ranking-hero"><div className="premium-ranking-icon">🏆</div><div><span>Reporte oficial</span><h2>Ranking de participantes</h2><p>Ranking actualizado de {phaseLabel}. La FASE 1 queda respaldada como histórico para el acumulado general final.</p></div></div><div className="premium-ranking-metrics"><article><span>Puntos {phaseLabel}</span><strong>{leader?`${leader.score.totalPoints} / ${maxPoints}`:`0 / ${maxPoints}`} <small>pts</small></strong><p>{leader?`${leader.percent}% de aciertos del líder`:'Sin puntaje registrado'}</p></article><article><span>Ganador / Empate</span><strong>{winnerTotal}</strong><p>Puntos por ganador directo o empate en penales del líder</p></article><article><span>Score exacto</span><strong>{scoreTotal}</strong><p>Marcadores exactos del líder</p></article><article><span>{showPenalty?'Bonus penales':'Partidos evaluados'}</span><strong>{showPenalty?penaltyTotal:evaluated}</strong><p>{showPenalty?`${evaluated} enfrentamientos evaluados`:`${rows.length} participantes · ${confirmed} confirmados`}</p></article></div><div className="premium-ranking-table"><div className="premium-ranking-table-head"><span>Pos</span><span>Participante</span><span>Puntos</span><span>Porcentaje</span><span>Estado</span></div>{rows.length?rows.map((row,index)=><div key={row.id || row.name} className={`premium-ranking-line ${index===0?'leader':''}`}><span className="premium-pos">{rankMedal(index)}</span><div className="premium-participant"><i>{participantInitials(row.name)}</i><div><strong>{row.name}</strong><small>Ganador/Empate: {row.score.winnerPoints || 0} · Score: {row.score.scorePoints || 0}{showPenalty?` · Penales: ${row.score.penaltyPoints || 0}`:''}</small></div></div><b>{row.score.totalPoints} pts</b><em>{row.percent}%</em><span className={`premium-status ${row.statusInfo.cls}`}>{row.statusInfo.label}</span></div>):<p className="premium-ranking-empty">No hay participantes disponibles para mostrar.</p>}</div></section>
}
...
const PHASE32_DAILY_LOCK_HOUR_EC = 14;
const PHASE32_DAILY_LOCK_MINUTE_EC = 0;

function phase32EcuadorDateTime(date, time='00:00'){
  const [year,month,day] = String(date).split('-').map(Number);
  const [hour,minute] = String(time || '00:00').split(':').map(Number);
  return new Date(Date.UTC(year, month - 1, day, (hour || 0) + 5, minute || 0, 0));
}

function phase32DailyLockDateTime(match){
  return phase32EcuadorDateTime(match.date, `${String(PHASE32_DAILY_LOCK_HOUR_EC).padStart(2,'0')}:${String(PHASE32_DAILY_LOCK_MINUTE_EC).padStart(2,'0')}`);
}

function phase32KickoffDateTime(match){
  return phase32EcuadorDateTime(match.date, match.time || '00:00');
}

function phase32MatchDeadline(match){
  const dailyLock = phase32DailyLockDateTime(match);
  const kickoff = phase32KickoffDateTime(match);
  return kickoff.getTime() < dailyLock.getTime() ? kickoff : dailyLock;
}

function phase32MatchLocked(match, appSettings, status){
  if (status === 'confirmed') return true;
  return Date.now() >= phase32MatchDeadline(match).getTime();
}

function phase32DeadlinePassed(matches=[]){
  return matches.some(match => Date.now() >= phase32MatchDeadline(match).getTime());
}

function phase32OpenMatches(matches=[], appSettings, status){
  return matches.filter(match => !phase32MatchLocked(match, appSettings, status));
}

function phase32ClosedMatches(matches=[], appSettings, status){
  return matches.filter(match => phase32MatchLocked(match, appSettings, status));
}

function formatPhase32Deadline(match){
  if (!match) return '—';
  const deadline = phase32MatchDeadline(match);
  return deadline.toLocaleString('es-EC', {
...
function phase32NextDeadline(matches=[], appSettings, status){
  const open = phase32OpenMatches(matches, appSettings, status)
    .map(match => ({ match, deadline: phase32MatchDeadline(match) }))
    .sort((a,b)=>a.deadline-b.deadline);
  return open[0] || null;
}

function phase32Locked(appSettings, status, matches=[]){
  if (status === 'confirmed') return true;
  return matches.length > 0 && matches.every(match => phase32MatchLocked(match, appSettings, status));
}
...
function setPhase32Field(setPredictions, matchId, field, value, match=null, appSettings=null, status='empty'){
  if (match && appSettings && phase32MatchLocked(match, appSettings, status)) return;
...
function Phase32PredictionView({participant,matches,predictions,setPredictions,realScores,setRealScores,score,status,appSettings,persist,penalties={}}){
  const locked = phase32Locked(appSettings,status,matches);
  const nextDeadline = phase32NextDeadline(matches, appSettings, status);
  const openCount = phase32OpenMatches(matches, appSettings, status).length;
  const closedCount = phase32ClosedMatches(matches, appSettings, status).length;
  ...
  return <section className="phase32-page">
    <div className="phase32-hero panel">
      <div>
        ...
        <h2>Pronóstico 16°</h2>
        <p className="phase32-deadline">Bloqueo diario a las <b>14:00 hora Ecuador</b>. Los partidos del día se bloquean a esa hora; si un partido inicia antes, se bloquea antes del inicio. Los encuentros de días posteriores siguen abiertos mientras el usuario no confirme el Pronóstico 16°.</p>
      ...
      <div className={`phase32-lock-card ${locked?'locked':'open'}`}>
        ...
        <span>{status === 'confirmed' ? 'Pronóstico confirmado' : nextDeadline ? `Próximo cierre: ${formatPhase32Deadline(nextDeadline.match)}` : 'Todos los partidos bloqueados'}</span><small>{openCount} abiertos · {closedCount} bloqueados</small>
      </div>
    </div>

    <div className="phase32-summary">
      <article><span>Puntos 16°</span><strong>{score.totalPoints} / {score.evaluatedMatches*3 || 48}</strong><small>pts</small></article>
      <article><span>Ganador / Empate</span><strong>{score.winnerPoints}</strong><small>aciertos</small></article>
      <article><span>Resultado</span><strong>{score.scorePoints}</strong><small>exactos</small></article>
      <article><span>Penales</span><strong>{score.penaltyPoints}</strong><small>bonus</small></article>
    </div>
    ...
    <div className="phase32-matches">
      {matches.map((match,index)=>{
        ...
        const ev = evaluatePhase32Prediction(match,predictions,realScores,penalties);
        const matchLocked = phase32MatchLocked(match, appSettings, status);
        ...
        return <article className={`phase32-match-card ${matchLocked ? "match-locked" : "match-open"}`} key={match.id}>
          ...
          <div className="phase32-fixture-meta"><span>{formatPhase32Date(match.date)} · {match.time}</span><b>{match.stadium}</b><em className={matchLocked ? "match-lock-chip locked" : "match-lock-chip open"}>{matchLocked ? "Bloqueado" : `Cierra: ${formatPhase32Deadline(match)}`}</em></div>
          <div className="phase32-score-inputs">
            <input type="number" min="0" max="30" disabled={matchLocked} value={prediction.homeGoals ?? ''} onChange={e=>setPhase32Field(setPredictions,match.id,'homeGoals',e.target.value,match,appSettings,status)} />
            <span>:</span>
            <input type="number" min="0" max="30" disabled={matchLocked} value={prediction.awayGoals ?? ''} onChange={e=>setPhase32Field(setPredictions,match.id,'awayGoals',e.target.value,match,appSettings,status)} />
          </div>
          {tie && <div className="phase32-penalties"><small>Ganador por penales</small><div><button type="button" disabled={matchLocked} className={prediction.penaltyWinner===match.home?'active':''} onClick={()=>setPhase32Field(setPredictions,match.id,'penaltyWinner',match.home,match,appSettings,status)}>{TEAMS[match.home]?.name || match.home}</button><button type="button" disabled={matchLocked} className={prediction.penaltyWinner===match.away?'active':''} onClick={()=>setPhase32Field(setPredictions,match.id,'penaltyWinner',match.away,match,appSettings,status)}>{TEAMS[match.away]?.name || match.away}</button></div></div>}
          ...
          {ev.penalized && <div className="phase32-penalty-alert">Penalizado: 0 puntos · {ev.penaltyReason}</div>}<div className="phase32-hits"><span>Ganador <b className={ev.winnerHit===null?'pending-hit':ev.winnerHit?'hit-ok':'hit-bad'}>{hitIcon(ev.winnerHit)}</b></span><span>Resultado <b className={ev.scoreHit===null?'pending-hit':ev.scoreHit?'hit-ok':'hit-bad'}>{hitIcon(ev.scoreHit)}</b></span><span>Penales <b className={ev.penaltyHit===null?'pending-hit':ev.penaltyHit?'hit-ok':'hit-bad'}>{hitIcon(ev.penaltyHit)}</b></span></div>
        </article>
      })}
    </div>
...
function ReportView({participant,matches,predictions,realScores,rankingRows=[],phase32Matches=[],phase32RealScores={}}){
  ...
  const phase32Rows = buildPremiumPhase32RankingRows(rankingRows,phase32Matches,phase32RealScores,phase32Penalties);
  ...
  return <div className="report-stack report-tabs-layout">
    <section className="report-tabs-shell report-tabs-3" aria-label="Opciones de reporte">
      <button type="button" className={reportTab==='ranking16'?'active':''} onClick={()=>setReportTab('ranking16')}>
        <Trophy size={16}/>
        <span>Ranking FASE 2</span>
      </button>
      <button type="button" className={reportTab==='historico1'?'active':''} onClick={()=>setReportTab('historico1')}>
        <BarChart3 size={16}/>
        <span>Histórico FASE 1</span>
      </button>
    ...
    {reportTab==='ranking16' && <PremiumRankingReport rows={phase32Rows} phaseLabel="FASE 2 · Pronóstico 16°" showPenalty={true}/>}

    {reportTab==='historico1' && <PremiumRankingReport rows={rankedRows} phaseLabel="FASE 1" showPenalty={false}/>}
    ...
    {reportTab==='pronosticos' && <section className="panel report private-report report-tab-panel">
      ...
      <div className="score-summary">
        ...
        <div><span>Ganador / Empate</span><strong>{score.winnerPoints}</strong></div>
...
async function removeSelected(){
  ...
  async function persistPhase32RealScore(match){
    const draft = phase32ScoreDraft[match.id] || {};
    const current = phase32RealScores[match.id] || {};
    const homeGoals = draft.homeGoals ?? current.homeGoals ?? '';
    const awayGoals = draft.awayGoals ?? current.awayGoals ?? '';
    const penaltyWinner = draft.penaltyWinner ?? current.penaltyWinner ?? null;
    ...
    try {
      ...
      setPhase32ScoreDraft(prev => ({...prev, [match.id]: {}}));
      setMessage('Resultado real FASE 2 actualizado.');
    } catch (ex) {
      setMessage(ex.message || 'No se pudo guardar el resultado real FASE 2.');
    }
  }

  const detail=selected?.forecast?.predictions || {};
  const phase32Detail=selected?.phase32Forecast?.predictions || {};
  const selectedScore = selected ? calculateParticipantScore(matches,detail,realScores) : null;
  const selectedPhase32Score = selected ? calculatePhase32Score(phase32Matches,phase32Detail,phase32RealScores) : null;
  const sortedRows = [...rows].sort((a,b)=>{
    const scoreA = calculatePhase32Score(phase32Matches,a.phase32Forecast?.predictions || {},phase32RealScores).totalPoints;
    const scoreB = calculatePhase32Score(phase32Matches,b.phase32Forecast?.predictions || {},phase32RealScores).totalPoints;
  ...
  return <section className="admin-layout"><div className="panel admin-controls"><h2><ShieldCheck/> Controles de fase</h2><div className="control-grid"><div><b>Registro de nuevos usuarios</b><span>{appSettings.registrationEnabled ? 'Abierto' : 'Cerrado'}</span></div><button className="ghost" disabled={busy} onClick={()=>runPhaseAction('set_registration_enabled', !appSettings.registrationEnabled)}>{appSettings.registrationEnabled ? 'Inhabilitar registros' : 'Habilitar registros'}</button><div><b>FASE 1 archivada</b><span>Histórico cerrado · solo consulta</span></div><button className="ghost archived-action" disabled title="La FASE 1 concluyó y queda respaldada para el acumulado general.">Archivada</button><div><b>Pronóstico 16°</b><span>{appSettings.phase32PredictionsUnlocked ? 'Habilitado por ADMIN' : 'Bloqueo diario 14:00 Ecuador'}</span></div><button className={appSettings.phase32PredictionsUnlocked ? 'danger' : 'ghost'} disabled={busy} onClick={()=>runPhaseAction('set_phase32_unlocked', !appSettings.phase32PredictionsUnlocked)}>{appSettings.phase32PredictionsUnlocked ? 'Bloquear Pronóstico 16°' : 'Habilitar Pronóstico 16°'}</button></div><p className="muted">La administración activa corresponde a FASE 2 · Pronóstico 16°. La FASE 1 queda archivada como histórico para el acumulado general.</p></div><div className="panel"><h2><Users/> Participantes registrados</h2><div className="participant-sort"><label>Ordenar por</label><select value={participantSort} onChange={e=>setParticipantSort(e.target.value)}><option value="points_desc">Puntos: mayor a menor</option><option value="points_asc">Puntos: menor a mayor</option><option value="name_asc">Nombre: A-Z</option><option value="name_desc">Nombre: Z-A</option></select></div><div className="admin-share-actions"><button className="primary share-ranking-btn" disabled={!rows.length} onClick={shareRanking}><Share2 size={16}/> Compartir ranking</button></div><div className="participant-list">{sortedRows.map(r=>{const rowScore=calculatePhase32Score(phase32Matches,r.phase32Forecast?.predictions || {},phase32RealScores); return <button key={r.id} onClick={()=>setSelected(r)} className={selected?.id===r.id?'selected':''}><b>{r.name}</b><span>{r.role} · FASE 2: {r.phase32Forecast?.confirmed ? 'Confirmado' : r.phase32Forecast?.status === 'draft' ? 'Borrador' : 'Sin pronóstico'} · {rowScore.totalPoints} pts · {scorePercentPhase(rowScore)}%</span></button>})}</div></div><div className="panel"><div className="admin-title-row"><h2>Detalle</h2><button className="ghost" disabled={busy} onClick={syncNow}>Recalcular puntajes</button></div>{!selected ? <p className="muted">Seleccione un participante para consultar sus pronósticos.</p> : <><div className="admin-detail-head"><div><p><b>{selected.name}</b> · clave: {selected.uniqueKey}</p>{selectedPhase32Score && <p className="muted">Puntos FASE 2: <b>{selectedPhase32Score.totalPoints} pts · {scorePercentPhase(selectedPhase32Score)}%</b> · Ganador/Empate: {selectedPhase32Score.winnerPoints} · Score: {selectedPhase32Score.scorePoints} · Penales: {selectedPhase32Score.penaltyPoints}</p>}{selectedScore && <p className="muted">Histórico FASE 1 respaldado: <b>{scoreShortLabel(selectedScore)}</b> · Ganador: {selectedScore.winnerPoints} · Score: {selectedScore.scorePoints}</p>}</div>{selected.role !== 'admin' && <button className="danger" disabled={busy} onClick={removeSelected}>Eliminar usuario y pronóstico</button>}</div>{message && <p className="admin-message">{message}</p>}<details open className="result-admin phase32-admin-real"><summary>Actualizar resultados reales FASE 2 · Pronóstico 16° <span className="admin-only-badge">Solo ADMIN</span></summary><div className="real-admin-list">{phase32Matches.map(match=>{const current=phase32RealScores[match.id] || {}; const draft=phase32ScoreDraft[match.id] || {}; const h=draft.homeGoals ?? current.homeGoals ?? ''; const a=draft.awayGoals ?? current.awayGoals ?? ''; const isTie=h!=='' && a!=='' && h!=null && a!=null && Number(h)===Number(a); const pw=draft.penaltyWinner ?? current.penaltyWinner ?? ''; return <div className="real-admin-row phase32-real-admin-row" key={match.id}><span>{match.matchNo}</span><b>{label(match.home)} vs {label(match.away)}</b><input type="number" min="0" max="99" value={h} onChange={e=>setPhase32ScoreDraft(prev=>({...prev,[match.id]:{...(prev[match.id]||{}),homeGoals:e.target.value}}))}/><span>:</span><input type="number" min="0" max="99" value={a} onChange={e=>setPhase32ScoreDraft(prev=>({...prev,[match.id]:{...(prev[match.id]||{}),awayGoals:e.target.value}}))}/>{isTie && <select value={pw} onChange={e=>setPhase32ScoreDraft(prev=>({...prev,[match.id]:{...(prev[match.id]||{}),penaltyWinner:e.target.value}}))}><option value="">Penales...</option><option value={match.home}>{TEAMS[match.home]?.name || match.home}</option><option value={match.away}>{TEAMS[match.away]?.name || match.away}</option></select>}<button className="ghost" onClick={()=>persistPhase32RealScore(match)}>Guardar</button></div>})}</div></details><details className="result-admin phase1-archive-admin"><summary>Archivo histórico FASE 1 · Score real <span className="admin-only-badge">Solo ADMIN</span></summary><div className="real-admin-list">{matches.map(m=>{const current=realScores[m.id] || {}; const draft=scoreDraft[m.id] || {}; return <div className="real-admin-row" key={m.id}><span>{m.matchNo}</span><b>{label(m.home)} vs {label(m.away)}</b><input type="number" min="0" max="99" value={draft.homeGoals ?? current.homeGoals ?? ''} onChange={e=>setScoreDraft(prev=>({...prev,[m.id]:{...(prev[m.id]||{}),homeGoals:e.target.value}}))}/><span>:</span><input type="number" min="0" max="99" value={draft.awayGoals ?? current.awayGoals ?? ''} onChange={e=>setScoreDraft(prev=>({...prev,[m.id]:{...(prev[m.id]||{}),awayGoals:e.target.value}}))}/><button className="ghost" onClick={()=>persistRealScore(m.id)}>Guardar</button></div>})}</div></details><div className="phase32-admin-detail-list">{phase32Matches.map(match=>{const p=phase32Detail[match.id] || {}; const real=phase32RealScores[match.id]; const evalResult=evaluatePhase32Prediction(match,phase32Detail,phase32RealScores); return <div className="phase32-admin-detail-row" key={match.id}><strong>{match.matchNo} · {label(match.home)} vs {label(match.away)}</strong><span>Pronóstico: {p.homeGoals ?? '—'} : {p.awayGoals ?? '—'} {p.penaltyWinner ? `· Penales: ${TEAMS[p.penaltyWinner]?.name || p.penaltyWinner}` : ''}</span><span>Real: {formatPhase32Real(real,match)}</span><small>Pts: {evalResult.points || 0} · G/Emp:{hitIcon(evalResult.winnerHit)} · Score:{hitIcon(evalResult.scoreHit)} · Pen:{hitIcon(evalResult.penaltyHit)}</small></div>})}</div><h3 className="admin-subtitle">Histórico FASE 1 respaldado</h3><div className="report-groups compact admin-standings-grid">{GROUPS.map(g=><div className="report-card" key={g.id}><h3>Grupo {g.id}</h3><Standings standings={calculateStandings(g.id,matches,detail)}/></div>)}</div></>}</div>{showSharePreview && <div className="share-preview-overlay" role="dialog" aria-modal="true"><div className="share-preview-card"><div className="share-preview-head"><div><span>Vista previa</span><h3>Ranking para compartir</h3></div><button className="ghost" onClick={()=>setShowSharePreview(false)}>Cerrar</button></div><pre>{shareText}</pre><div className="share-preview-actions"><button className="ghost" onClick={async()=>{await navigator.clipboard?.writeText(shareText); setMessage('Ranking copiado al portapapeles.');}}>Copiar texto</button><button className="primary" onClick={confirmShareRanking}><Share2 size={16}/> Compartir</button></div></div></div>}</section>
}
function Watermark() {
  return (

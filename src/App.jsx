...
import { getForecast, getMatches, listParticipantsWithForecasts, loginOrCreateParticipant, saveForecast, supabase } from './lib/storage';
import './styles.css';
...
function AdminView({matches}){ const [rows,setRows]=useState([]); const [selected,setSelected]=useState(null); useEffect(()=>{ listParticipantsWithForecasts().then(setRows); },[]); const detail=selected?.forecast?.predictions || {}; return <section className="admin-layout"><div className="panel"><h2><Users/> Participantes registrados</h2><div className="participant-list">{rows.map(r=><button key={r.id} onClick={()=>setSelected(r)} className={selected?.id===r.id?'selected':''}><b>{r.name}</b><span>{r.role} · {r.forecast?.confirmed?'Confirmado':'Sin confirmar'}</span></button>)}</div></div><div className="panel"><h2>Detalle</h2>{!selected ? <p className="muted">Seleccione un participante para consultar sus pronósticos.</p> : <><p><b>{selected.name}</b> · clave: {selected.uniqueKey}</p><div className="report-groups compact">{GROUPS.map(g=><div className="report-card" key={g.id}><h3>Grupo {g.id}</h3><Standings standings={calculateStandings(g.id,matches,detail)}/></div>)}</div></>}</div></section> }
